            ...
            if game.valid_guess(u_inp) == True:
                ...
                if 'P' in str(control).upper():
                    ...
                    for c,b in zip(game.colours,game.board):
                        ...
                        guess_string="".join(b)
                        if guess_string != colors_string:  # simpleTt hack to not print blank lines: color string is never a legit word. so if both are equal then its an empty line (we haven't played it yet).
